...
本目录新增了一个基于 PowerShell + Windows 托盘的 runner 管理程序：

- [runner-tray.ps1](C:/actions-runner/runner-tray.ps1)：主程序
- [runner-tray.cmd](C:/actions-runner/runner-tray.cmd)：双击即可启动的入口
- [run.md](C:/actions-runner/run.md)：使用说明

## 功能

1. 托盘菜单可启动 / 停止本机 GitHub Actions runner
2. 可勾选 **Run on Windows startup** 设置当前用户开机自启动
3. 托盘图标会按 runner 状态显示不同标记：
   - 红色斜线：已停止
   - 绿色圆点：Idle（仅 `Runner.Listener.exe` 存在）
   - 橙色圆环：Busy（检测到 `Runner.Worker.exe`）
4. 支持查看 **run.cmd 实时输出**（不改 [run.cmd](C:/actions-runner/run.cmd) 本体）
...
- [runner-tray.cmd](C:/actions-runner/runner-tray.cmd)
...
- **Open latest runner log**：打开最新的 runner 诊断日志（`_diag/Runner_*.log`）
...
- runner 的后台宿主状态文件保存在 [\.trayicon/](C:/actions-runner/.trayicon)
- 后台宿主日志文件为 [runner-host.log](C:/actions-runner/.trayicon/runner-host.log)
